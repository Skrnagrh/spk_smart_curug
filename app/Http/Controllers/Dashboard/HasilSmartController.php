...
class HasilSmartController extends Controller
{
    ...
    protected function calculate()
    {
        ...
        $checkHasEmptyData = false; // Flag untuk memeriksa apakah ada nilai bobot yang kosong

        for ($i = 0; $i < count($nilaiBobotGroupByAlternatifId); $i++) {
            $HasilPerhitungan = 0;

            for ($j = 0; $j < count($persentaseBobot); $j++) {
                if (isset($matrixTernormalisasi[$i][$j]) && isset($matrixTernormalisasi[$i][$j]['hasil_utiliti'])) {
                    $HasilPerhitungan += $persentaseBobot[$j]['persentase_bobot'] * $matrixTernormalisasi[$i][$j]['hasil_utiliti'];
                } else {
                    // Set flag jika ada nilai bobot yang kosong
                    $checkHasEmptyData = true;
                    break;
                }
            }
            ...
            $result[] = [
                'alternatif_code' => $nilaiBobotGroupByAlternatifId[$i]->code,
                'code_curug' => $nilaiBobotGroupByAlternatifId[$i]->code_curug,
                'name_curug' => $nilaiBobotGroupByAlternatifId[$i]->name_curug,
                'hasil_perh' => $HasilPerhitungan
            ];
        }
